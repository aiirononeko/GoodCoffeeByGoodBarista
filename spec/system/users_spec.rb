...
describe 'ユーザー登録機能', type: :system do
  ...
  describe 'バリスタユーザー一覧表示機能' do
    before do
      ...
      visit new_user_registration_path
      fill_in '名前', with: 'バリスタ２'
      choose '男性'
      select '1997', from: 'user_birthday_1i'
      select '10', from: 'user_birthday_2i'
      select '19', from: 'user_birthday_3i'
      fill_in 'メールアドレス', with: 'barista2@example.com'
      fill_in 'パスワード', with: 'password'
      fill_in 'パスワード（確認）', with: 'password'
      click_button '登録する'
    end
    context 'バリスタユーザー一覧画面に移動した時' do
      before do
        visit users_path
      end

      it 'バリスタユーザーが表示される' do
        expect(page).to have_content 'バリスタ'
        expect(page).to have_content 'バリスタ２'
      end
    end
  end
end